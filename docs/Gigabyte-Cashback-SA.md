...
版本：0.7 Spec Aligned／2026-09-10。

UAT 後續決策：後台加入繁體中文切換並記住偏好；依使用者明確要求移除 UAT 入口密碼，任何訪客可按模擬登入操作後台。正式身份整合需求維持不變。詳見 [公開 UAT 與後台語系](Admin-Language-and-Open-UAT.md)。

依 Spec v0.3 更新：未送出 Draft 開啟及送出前檢查最新發布版本，提示確認後保留資料／附件套用並重設必要同意；其他階段固定送出時版本。前台支援繁體中文，送出錯誤在操作區自動定位。詳見 [草稿版本與語系](Draft-Version-and-Language.md)。
